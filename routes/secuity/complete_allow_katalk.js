...
module.exports = async (req, res) => {
  if(req.user) {
    try {
      ...
      if ((await sqlp(sql, SqlString.format("SELECT * FROM katalk WHERE id=?", [req.user.id]))).length != 0) {
        throw ('이미 카카오톡 알림 서비스에 등록되어 있습니다.')
      }
      var data = await sqlp(sql, SqlString.format("SELECT * FROM 2fa WHERE phone=?", [phone]))
      if (data.length == 0) {
        throw ('인증번호를 요청한적이 없습니다.')
      }
      if (data[0]['code'] != code) {
