...
function complete(req, res){
  return new Promise(function (resolve, reject) {
    if(req.user) {
      ...
      sql(SqlString.format("SELECT * FROM katalk WHERE id=?", [req.user.id]), function(err,rows){
        if(err) { return reject("1번 질의 오류") }
        if(rows.length != 0){
          return reject({ success: false, title: "부가서비스 가입 실패", message: "이미 부가서비스를 이용하고 있습니다."})
        }
        sql(SqlString.format("SELECT * FROM 2fa WHERE id=?", [req.user.id]), function(err,rows2){
          ...
          sql(SqlString.format('insert into katalk values (?, ?)', [req.user.id, phone]), function(err, rows3){
