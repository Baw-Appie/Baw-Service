  ...
  if(req.user) {
    try {
      ...
      if(service == "1" || service == "2"){
        var data = (await sqlp(sql, SqlString.format("SELECT * from service"+service+" WHERE owner=? and num=?", [req.user.id, id])))
        if (data.length == 0) {
            ...
            switch(service) {
              case "1":
                ...
                if (api['api_type'] == "HTTP") {
                  await sqlp(sql, SqlString.format('insert into api1 values (?, ?, ?, ?, ?, ?, ?)', [req.user.id, api['api_key'], data['page'], data['nick'], data['bal'], data['pin'], api_cmd]))
                }
                break;
              case "2":
                api_cmd = api_cmd.replace("<player>", data['nick']);
        ...
        try { await sqlp(sql, SqlString.format('UPDATE `service'+req.params.service+'` SET status=? WHERE num=?', [req.params.status, req.params.id])) } catch { return res.json({ success: false, title: '실패했습니다.', message: "요청에 실패했습니다. 좌측 메뉴의 버그 신고로 이 문제를 신고하세요." }) }
