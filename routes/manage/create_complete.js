    ...
    if(req.user) {
      ...
      sql.query(SqlString.format("SELECT * FROM pages WHERE owner=? and service=?", [req.user.id, service]), function(err,rows){
        ...
        sql.query(SqlString.format("SELECT * FROM pages WHERE name=?", [name]), function(err,rows2){
          ...
          if(service == 1) {
            var sql_Request = SqlString.format("INSERT INTO pages SET name=?, owner=?, service=1, status=1, notice='아래 정보를 입력해주시면 최대한 빠르게 처리해 드리겠습니다.', pagedata=json_object('regdate', ?, 'sms_ok', 0, 'mail_ok', 1, 'tg_ok', 0, 'kakao_ok', 0, 'bouns', '없음', 'api_cmd', '', 'disabled', '', 'theme', 'bootstrap3', 'youtube', '')", [name, req.user.id, date])
          ...
          if(service == 2) {
            var sql_Request = SqlString.format("INSERT INTO pages SET name=?, owner=?, service=2, status=1, notice='아래 정보 입력 후 전송시 전송된 데이터는 정품 인증 사이트에 전송되며 이때 저장되지 않고 바로 Mojang에 연결하여 정품 아이디가 맞는지 확인 후 아이디와 IP주소만 저장됩니다.', pagedata=json_object('regdate', ?, 'mail_ok', 1, 'api_cmd', '', 'theme', 'bootstrap3', 'auto_process', 0)", [name, req.user.id, date])
          ...
          if(service == 3) {
            var sql_Request = SqlString.format("INSERT INTO pages SET name=?, owner=?, service=3, status=1, notice='이 정보는 부정확할 수 있으니 직접 접속하여 확인해보시기 바랍니다.', pagedata=json_object('regdate', ?, 'sv_ip', '', 'sv_port', 25565)", [name, req.user.id, date])
          ...
          if(service == 4) {
            var sql_Request = SqlString.format("INSERT INTO pages SET name=?, owner=?, service=4, status=1, notice='', pagedata=json_object('regdate', ?)", [name, req.user.id, date])
