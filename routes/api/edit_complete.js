  ...
  if (req.user && service) {
    switch (service) {
      case "API":
        ...
        var sql_Request = SqlString.format("UPDATE `api` SET ? WHERE id=?", [{ api_enable, api_key, api_type, api_ip, api_port }, req.user.id])
        break;
      case "SMS":
        var { phone } = req.body
      ...
      case "Custom":
        ...
        var sql_Request = SqlString.format('UPDATE `custom_domain` SET `domain` = ? WHERE `owner` = ?', [domain, req.user.id])
        break;
      default:
        return res.json({ success: false, title: "설정할 수 없습니다.", message: "이미 동일한 도메인이 등록되어 있습니다." });
    }
