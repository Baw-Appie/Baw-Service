...
module.exports = async (req, res) => {
  if(req.body.api_key && req.body.id){
    ...
    try {
      var donation = await sqlp(sql, SqlString.format("SELECT * FROM api1 WHERE owner=? and api_key=?", [id, api_key]))
      var id_check = await sqlp(sql, SqlString.format("SELECT * FROM api2 WHERE owner=? and api_key=?", [id, api_key]))
